...
for alpha in [0.01]:
	for muP in [0.1]:
		...
		meanNumber = colnum.mean()
		stdNumber = colnum.std()
		steadyClones = calculateSteadyClones(alpha, omega, muP, p, 
		                                   numOfClones, boneCapacity)

		plt.figure()

		output = plt.hist(colnum, bins=50, histtype='stepfilled', alpha=0.8)
		...
		plt.ylabel("Counts")
		plt.title("Distribution of total number of clones")
		...
		plt.axvline(x=steadyClones, c='g', linewidth=2,linestyle='--', label='Steady Number=%.2f'%steadyClones)

		plt.legend(loc=2)
		plt.savefig("HistClonesCounts_alpha{0}_p{1}_mu{2}_boneCap{3}.png".format(alpha, p, muP+omega,
		                boneCapacity)) 
		...
		plt.axhline(y=steadyClones, c='g', linestyle='--', label='steady state')
